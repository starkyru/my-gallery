...
import type { Tag } from '@gallery/shared';
...
export default function AdminTagsPage() {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      await api.tags.create({ name: newName.trim(), slug: deriveSlug(newName) }, token);
      setNewName('');
  ...
  async function handleSaveEdit(id: number) {
    ...
    try {
      await api.tags.update(id, { name: editName.trim(), slug: deriveSlug(editName) }, token);
      setEditingId(null);
  ...
  async function handleDelete(id: number) {
    ...
    try {
      await api.tags.delete(id, token);
      loadData();
    } catch (e: unknown) {
      notify.error(e instanceof Error ? e.message : 'Failed to delete tag');
    }
  }
  ...
  return (
    <div>
      <h1 className="font-serif text-3xl mb-8">Tags</h1>

      <form onSubmit={handleCreate} className="mb-8 flex gap-3 items-end">
        <div className="flex-1">
          <label className="block text-xs text-gallery-gray mb-1">New Tag</label>
          <input
            value={newName}
      ...
      <div className="border border-white/10 rounded-lg overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {tags.map((tag) => (
              <tr key={tag.id} className="border-b border-white/5">
                ...
                <td className="px-4 py-3 text-gallery-gray font-mono text-xs">{tag.slug}</td>
                <td className="px-4 py-3 text-center">{tag.imageCount ?? 0}</td>
                    ...
                    {editingId === tag.id ? (
                      ...
                      <>
                        ...
                        <button
                          ...
                          title={
                            (tag.imageCount ?? 0) > 0 ? 'Has images — cannot delete' : 'Delete'
                          }
                        >
                          Delete{(tag.imageCount ?? 0) > 0 ? ' (has images)' : ''}
                        </button>
                      </>
                    )}
                  </div>
        ...
        {tags.length === 0 && <p className="text-center text-gallery-gray py-8">No tags yet.</p>}
      </div>
    </div>
  );
}
